...
export class GameServer {
  ...
  private playerBoostState: Map<
    string,
    { active: boolean; charge: number; remaining: number }
  > = new Map(); // playerId -> boost state
  private lastBoostRequested: Map<string, boolean> = new Map(); // playerId -> último estado de boost solicitado

  // Sistema de rondas
  private readonly TOTAL_ROUNDS: number = 5; // 5 rondas por partida
  ...
  start(sendInitialState: boolean = false): void {
    ...
    if (sendInitialState) {
      logger.log(
        `📡 Enviando estado inicial con ${this.gameState.players.length} jugador(es)`
      );
  ...
  private processInputs(): void {
    ...
    for (const player of players) {
      ...
      if (inputs.length > 0) {
        ...
        if (inputs.length > 1) {
          logger.log(
            `📥 Procesando ${inputs.length} inputs de ${player.name}, usando el más reciente`
          );
        }
  ...
  private updatePlayers(deltaTime: number): void {
    ...
    for (const player of players) {
      ...
      if (this.gameState.tick % 60 === 0) {
        ...
        const timeSinceLastPoint =
          lastPointTime !== undefined ? normalizedTimer - lastPointTime : 0;
        logger.log(
          `🔍 [${player.id.substring(0, 8)}] Tick ${
            this.gameState.tick
          } | Timer: ${normalizedTimer.toFixed(
            2
          )}ms | timeInCycle: ${timeInCycle.toFixed(
            2
          )}ms | shouldDraw: ${shouldDrawTrail} | wasDrawing: ${wasDrawingTrail}`
        );
        logger.log(
          `   Trail: ${validPoints} válidos, ${nullPoints} nulls, total: ${trailLength} | Tiempo desde último punto: ${timeSinceLastPoint.toFixed(
            2
          )}ms`
        );
        ...
        if (validPoints < 10 && this.gameState.tick > 300) {
          logger.warn(
            `   ⚠️ Trail muy corto después de ${this.gameState.tick} ticks!`
          );
        }
      }
        ...
        if (trailLengthBefore >= 1195 && this.gameState.tick % 60 === 0) {
          logger.log(
            `   ➕ [${player.id.substring(
              0,
              8
            )}] Punto agregado: trail antes=${trailLengthBefore}, después=${
              player.trail.length
            }`
          );
        }
      ...
      if (lastPointTime !== undefined) {
        ...
        if (timeSinceLastPoint < 0) {
          timeSinceLastPoint =
            normalizedTimer + (MAX_TIMER_VALUE - lastPointTime);
        }
        ...
        if (timeSinceLastPoint > 1000 && !pointAdded) {
          logger.warn(
            `⚠️ [${player.id.substring(
              0,
              8
            )}] Timer anómalo detectado: sin puntos por ${timeSinceLastPoint.toFixed(
              2
            )}ms, timeInCycle=${timeInCycle.toFixed(
              2
            )}ms, shouldDraw=${shouldDrawTrail}, forzando dibujo`
          );
          logger.warn(
            `   Detalles: normalizedTimer=${normalizedTimer.toFixed(
              2
            )}ms, lastPointTime=${lastPointTime.toFixed(2)}ms, gapInterval=${
              this.gapInterval
            }ms, gapDuration=${this.gapDuration}ms`
          );
          player.trail.push({ ...player.position });
          this.playerLastPointTime.set(player.id, normalizedTimer);
      ...
      if (!pointAdded && shouldDrawTrail && this.gameState.tick % 60 === 0) {
        logger.warn(
          `⚠️ [${player.id.substring(
            0,
            8
          )}] shouldDrawTrail=true pero no se agregó punto! timeInCycle=${timeInCycle.toFixed(
            2
          )}ms`
        );
      }
      ...
      if (!hasValidPoints) {
        ...
        logger.warn(
          `⚠️ [${player.id.substring(
            0,
            8
          )}] Trail sin puntos válidos! Agregando punto de emergencia. Trail length: ${
            player.trail.length
          }, shouldDraw: ${shouldDrawTrail}, timeInCycle: ${timeInCycle.toFixed(
            2
          )}ms`
        );
        player.trail.push({ ...player.position });
      ...
      if (this.gameState.tick % 300 === 0) {
        ...
        logger.log(
          `   Timer: ${normalizedTimer.toFixed(
            ...
            2
          )}ms | shouldDraw: ${shouldDrawTrail}`
        );
        logger.log(
          `   Trail: ${trailStats.valid} válidos, ${trailStats.nulls} nulls, total: ${trailStats.total}`
        );
        logger.log(
          `   Último punto: ${trailStats.lastPointTime}ms | Tiempo desde último: ${trailStats.timeSinceLastPoint}ms`
        );
      }
    }
    ...
    if (this.gameState.tick % 60 === 0) {
      logger.log(
        `🎮 Tick ${this.gameState.tick} | Jugadores vivos: ${aliveCount}/${players.length}`
      );
    }
    ...
    if (this.gameState.tick % 300 === 0 && this.gameState.tick > 0) {
      ...
      const avgTrailLength =
        trailStats.length > 0
          ? (totalPoints / trailStats.length).toFixed(1)
          : 0;
      const maxTrailLength = Math.max(...trailStats.map((s) => s.total), 0);
      const minTrailLength = Math.min(...trailStats.map((s) => s.total), 0);

      logger.performance(
        `📈 RENDIMIENTO [Tick ${this.gameState.tick}] - Estadísticas de Trails:`
      );
      logger.performance(
        `   Total puntos: ${totalPoints} (${totalValidPoints} válidos, ${
          totalPoints - totalValidPoints
        } nulls)`
      );
      logger.performance(
        `   Promedio: ${avgTrailLength} puntos/jugador | Min: ${minTrailLength} | Max: ${maxTrailLength}`
      );
      trailStats.forEach((s) => {
        logger.performance(
          `   [${s.id}]: ${s.total} total (${s.valid} válidos, ${s.nulls} nulls)`
        );
      });
    }
  }
  ...
  private checkCollisions(): void {
    ...
    for (const player of players) {
      ...
      if (
        ...
        logger.log(
          `💀 Jugador ${player.name} (${player.id.substring(
            0,
            8
          )}...) murió por colisión con borde en (${player.position.x.toFixed(
            0
      ...
      if (currentPos && newPos) {
        ...
        if (trailCollision.collided) {
          ...
          logger.log(
            `💀 Jugador ${player.name} murió por colisión con trail de ${trailCollision.collidedWith}`
          );
          continue;
        }
        ...
        const selfCollision = checkSelfCollision(
          currentPos,
          newPos,
          player.trail
        );
        ...
        if (selfCollision) {
          ...
          logger.log(
            `💀 Jugador ${player.name} murió por colisión consigo mismo`
          );
        }

        // Log de rendimiento de colisiones (cada 5 segundos, solo si es lento)
        if (
          this.gameState.tick % 300 === 0 &&
          (trailCollisionTime > 1 || selfCollisionTime > 1)
        ) {
          logger.performance(
            `⏱️ [${player.id.substring(
              0,
              8
            )}] Tiempo colisiones: trail=${trailCollisionTime.toFixed(
              3
            )}ms, self=${selfCollisionTime.toFixed(3)}ms`
          );
        }
      }
    }
  ...
  private calculateRoundPoints(): void {
    ...
    deathOrder.forEach(({ playerId, points }) => {
      ...
      logger.log(
        ...
        }: +${points} puntos (Total: ${totalPoints})`
      );
    });
  }
  ...
  requestNextRound(): void {
    ...
    if (this.gameState.gameStatus !== "round-ended") {
      logger.log(
        `⚠️  Intento de solicitar siguiente ronda cuando el estado es ${this.gameState.gameStatus}`
      );
      return;
    }
    ...
    logger.log(
      `⏱️  Iniciando cuenta atrás para siguiente ronda: ${this.nextRoundCountdown} segundos`
    );
  ...
  private startNextRound(): void {
    try {
      ...
      logger.log(
        `🔄 [startNextRound] Iniciando ronda ${nextRound}/${this.TOTAL_ROUNDS}`
      );
      ...
      const allPlayers = this.playerManager.getAllPlayers();
      logger.log(
        `   [startNextRound] Reiniciando ${allPlayers.length} jugadores...`
      );
      allPlayers.forEach((player) => {
        player.alive = true;
        player.trail = [];
  ...
  addInput(input: GameInputMessage): void {
    ...
    if (queue.length % 10 === 0) {
      ...
      logger.log(
        `⌨️  Input recibido de ${player?.name || input.playerId}: ${
          input.key
        } (cola: ${queue.length})`
      );
    }
  ...
  private broadcastState(force: boolean = false): void {
    ...
    if (this.broadcastCallback) {
      ...
      if (this.gameState.tick % 60 === 0 || force) {
        ...
        logger.performance(
          `📡 Broadcast estado | Tick: ${this.gameState.tick} | Jugadores: ${
            alivePlayers.length
          }/${this.gameState.players.length} | Rate: ${
            1000 / (this.tickInterval * this.broadcastInterval)
          }Hz${force ? " (FORZADO - Estado final)" : ""}`
        );
      }
    }
  }
  ...
  initializePlayers(): void {
    ...
    players.forEach((player, index) => {
      ...
      const currentColorInUse = otherPlayers.some(
      ...
      if (player.color === "#ffffff" || currentColorInUse) {
        ...
        logger.log(
          `🎨 Asignando color ${player.color} a ${
            player.name
        ...
        logger.log(
          `✅ Preservando color ${player.color} de ${
            player.name
          } (${player.id.substring(0, 8)}...)`
        );
      }
